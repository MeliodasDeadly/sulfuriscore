 ...
 */
public class JerryCanClickListener extends SulfuVListener {
    ...
    private boolean makePlayerPay(double price) { //returns true if payed/doesn't have to, false if didn't pay/error
        if (!ConfigModule.defaultConfig.isFillJerryCanPriceEnabled())
            return true; //it isn't enabled, so just fill the jerrycan...

        //return DependencyModule.vault.withdrawMoneyPlayer(player, price); //@ToDo("IMPLEMENT MONEY")
        return true;
    }
